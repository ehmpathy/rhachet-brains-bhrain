...
/**
 * .what = chutes code generation integration tests for invokeBrainArch1
 ...
 * .note = skipped in CI: 50K token prompts + slow API response, timeout before completion
 */
describe.skip('invokeBrainArch1.chutes.codewrite', () => {
  const getContext = () => ({
    creds: {
      anthropic: {
  ...
  given('[case1] chutes with code generation from briefs', () => {
...
After you write it, use the test tool to verify it works with Austin, TX coordinates (--lat 30.2672 --lon -97.7431).`,
          ...
          logOutputHead({
            label: 'brainArch1.chutes.codegen',
