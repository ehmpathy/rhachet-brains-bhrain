...
/**
 * .what = chutes integration tests for invokeBrainArch1
 * .why = verify bhrain works with chutes BrainAtom (deepseek-v3.2)
 *
 * .note = requires CHUTES_API_KEY env var
 * .note = skipped: upstream atom needs plain text fallback fix (see handoff)
...
describe.skip('invokeBrainArch1.chutes', () => {
  ...
  beforeAll(async () => {
    testDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'brain-arch1-chutes-deepseekv32-test-'),
    );
  });
  ...
  given('[case1] chutes with simple prompt', () => {
      ...
      then.repeatably(REPEATABLY_CONFIG)(
        ...
        async () => {
          ...
          const config: BrainArch1Config = {
            atom: genAtomChutes({ slug: 'chutes/deepseek/v3.2' }),
            toolboxes: [],
          ...
          logOutputHead({
            label: 'brainArch1.chutes.simple',
  ...
  given('[case2] chutes with file read tool', () => {
    when('[t0] asked to read a file that exists', () => {
      then.repeatably(REPEATABLY_CONFIG)(
        ...
        async () => {
          ...
          const testFilePath = path.join(testDir, 'test-read-chutes.txt');
          await fs.writeFile(testFilePath, 'Hello from chutes test file!');
          ...
          logOutputHead({
            label: 'brainArch1.chutes.fileread',
            output: result.finalResponse ?? '',
          });
  ...
  given('[case3] chutes with multi-turn tool usage', () => {
    when('[t0] writes and then reads file', () => {
      then.repeatably(REPEATABLY_CONFIG)(
        ...
        async () => {
          ...
          logOutputHead({
            label: 'brainArch1.chutes.multiturn',
            output: result.finalResponse ?? '',
          });
